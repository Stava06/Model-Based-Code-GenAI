...
"You are the Supervisor's problem-resolution brain for an OPL-to-code "
            ...
            "- \"cant_solve\": The problem cannot be recovered automatically, including "
            "when the OPL specification's logic is incorrect and must be changed by the "
            "user. Set `user_error` to a clear, user-facing explanation of what the user "
            "needs to change.\n\n"
    ...
    def finish_and_return_user(self, tool_context: ToolContext) -> dict[str, Any]:
        ...
        # An unrecoverable workflow problem takes precedence over an artifact that
        # may have been generated before the problem was discovered.
        if problem:
        ...
        log_payload = {"status": result["status"], "message": result["message"]}
        if result["status"] == "success":
            success_message("SupervisorTools", log_payload)
        else:
            error_message("SupervisorTools", log_payload)
